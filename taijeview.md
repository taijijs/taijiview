...
# Taijiview
  ...
  The taijiview template engine for node.js

## Synopsis

    taijiview [-h|--help] [-v|--version] [-o|--obj STR]
         [-O|--out DIR] [-p|--path PATH] [-P|--pretty]
         [-c|--client] [-D|--no-debug]

## Examples

  translate taijiview the templates dir
  ...
  in your projects.
     ...
     $ taijiview --client --no-debug < my.taijiview
...
## Boolean attributes
  ...
  Boolean attributes are mirrored by Taijiview, and accept
...
## Block expansion
  ...
  Taijiview supports the concept of "block expansion", in which
  using a trailing ":" after a tag will inject a block:
  
    ul
      li > a: Foo; li > a: Bar
      li > div "Baz" > p: dsafjkl\; 
...
## Pipe text
  ...
  Using pipes we can also specify regular Taijiview tags
  within the text:
...
## Text only tags
  ...
  contain nested HTML so Taijiview implies the trailing ".":
...
## Template script tags
  ...
  tags using Taijiview, typically for client-side templates.
  ...
  _type_ attribute such as _text/x-template_:
    ...
    script text/template 
      h1: Look!
      p: Taijiview still works in here!
...
## Inline HTML
  ...
  in Taijiview can be annoying, luckily we can add plain
  HTML as well:
...
## Code
  ...
  To buffer output with Taijiview simply use _=_ at the beginning
  ...
  The final way to mess with JavaScript code in Taijiview is the unbuffered
  _-_, which can be used for conditionals, defining variables etc:
   ...
   Taking this further Taijiview provides some syntax for conditionals,
   iteration, switch statements etc. Let's look at those next!

## Assignment

  Taijiview's first-class assignment is simple, simply use the _=_
  operator and Taijiview will _var_ it for you. The following are equivalent:
...
## Conditionals
  ...
  Taijiview's first-class conditional syntax allows for optional
  ...
  Taijiview provides the negated version, _unless_ as well, the following
  are equivalent:
...
## Iteration
  ...
  JavaScript's _for_ loops don't look very declarative, so Taijiview
...
## Mixins
  ...
  Mixins provide a way to define taijiview "functions" which "mix in"
  their contents when called. This is useful for abstracting
  out large fragments of Taijiview.
